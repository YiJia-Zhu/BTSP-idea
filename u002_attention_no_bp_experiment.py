#!/usr/bin/env python3
"""
U002 Transformer-inspired no-BP next-token prototype.

This is not a 1B run.  It is a scalable small prototype for testing whether the
architecture direction is useful before allocating large memory/compute:
  - token neurons over a compact vocabulary;
  - sinusoidal position codes;
  - fixed random causal attention reads over the local context;
  - locally trained per-block feed-forward maps toward token target codes;
  - local target/wrong output-neuron updates;
  - finite-difference diagnostics for the output and block update directions.

No BP/BPTT, pretrained backbone, raw replay, answer slots, or n-gram tables are
used by the model.
"""
...
class U002AttentionNoBPModel:
    def __init__(self, vocab_size: int, cfg: U002Config) -> None:
    ...
    def parameter_count(self) -> int:
        arrays = [self.token_codes, self.position_codes, self.output_weights, self.output_counts, self.output_bias]
        arrays.extend(self.block_targets)
    ...
    def forward(self, context: np.ndarray, collect: bool = False) -> tuple[np.ndarray, list[dict[str, np.ndarray]]]:
        ...
        for block_idx in range(self.blocks):
            ...
            hidden_pre = self.ff_weights[block_idx] @ local_input + self.ff_biases[block_idx]
            hidden = np.tanh(hidden_pre).astype(np.float32)
            current = phase.normalize_vector(local_input + self.cfg.ff_scale * hidden)
            seq = seq.copy()
            seq[-1] = current
            if collect:
    ...
    def update(self, context: np.ndarray, target: int) -> None:
        ...
        self.update_output(feature, logits, target)
        self.update_blocks(traces, target)
    ...
    def update_output(self, feature: np.ndarray, logits: np.ndarray, target: int) -> None:
        target = int(target)
        self.output_weights[target] = phase.normalize_vector(self.output_weights[target] + self.cfg.output_lr * feature)
        if self.cfg.neg_k <= 0:
            return
        adjusted = logits.astype(np.float32, copy=True)
        target_score = float(adjusted[target])
        adjusted[target] = -np.inf
        k = min(max(int(self.cfg.neg_k), 1), self.vocab_size - 1)
        wrongs = np.argpartition(adjusted, -k)[-k:]
        for wrong in wrongs:
            wrong = int(wrong)
            if float(adjusted[wrong]) + self.cfg.output_margin <= target_score:
                continue
            self.output_weights[wrong] = phase.normalize_vector(
                self.output_weights[wrong] - (self.cfg.output_lr / k) * feature
            )

    def update_blocks(self, traces: list[dict[str, np.ndarray]], target: int) -> None:
        for block_idx, trace in enumerate(traces):
            local_input = trace["input"]
            hidden = trace["hidden"]
            local_target = self.block_targets[block_idx][int(target)]
            delta = (local_target - hidden) * (1.0 - np.square(hidden))
            self.ff_weights[block_idx] = (
                self.ff_weights[block_idx] + self.cfg.block_lr * np.outer(delta, local_input).astype(np.float32)
            )
            if self.cfg.block_bias_lr > 0.0:
                self.ff_biases[block_idx] = self.ff_biases[block_idx] + self.cfg.block_bias_lr * delta.astype(np.float32)
            if self.cfg.row_normalize:
                self.ff_weights[block_idx] = phase.normalize_rows(self.ff_weights[block_idx])


def estimate_transformer_like_params(vocab_size: int, d_model: int, blocks: int, ff_mult: int) -> int:
    vocab = max(int(vocab_size), 1)
    d = max(int(d_model), 1)
    b = max(int(blocks), 1)
    ff = max(int(ff_mult), 1)
    embeddings_and_head = 2 * vocab * d
    per_block_attention = 4 * d * d
    per_block_ffn = 2 * ff * d * d
    per_block_local_targets = vocab * d
    return int(embeddings_and_head + b * (per_block_attention + per_block_ffn + per_block_local_targets))


def run_pass(
    model: U002AttentionNoBPModel,
    ids: np.ndarray,
    update: bool,
    limit: int,
) -> dict[str, float | int]:
...
def collect_diag_batch(model: U002AttentionNoBPModel, ids: np.ndarray, batch_size: int) -> tuple[np.ndarray, np.ndarray]:
    features: list[np.ndarray] = []
    targets: list[int] = []
    order = model.context_len
    for idx in range(order, min(len(ids), order + max(int(batch_size), 0))):
        feature, _ = model.forward(ids[idx - order : idx], collect=False)
        features.append(feature)
        targets.append(int(ids[idx]))
    if not features:
        return np.zeros((0, model.d_model), dtype=np.float32), np.zeros(0, dtype=np.int64)
    return np.stack(features, axis=0).astype(np.float32), np.asarray(targets, dtype=np.int64)
...
def output_loss(weights: np.ndarray, bias: np.ndarray, features: np.ndarray, targets: np.ndarray, scale: float) -> float:
    if features.shape[0] == 0:
        return 0.0
    losses = []
    for feature, target in zip(features, targets):
        logits = scale * (weights @ feature) + bias
        probs = phase.softmax(logits.astype(np.float32), temperature=1.0)
        losses.append(-math.log(float(probs[int(target)]) + 1e-9))
    return float(np.mean(losses))


def local_output_step(model: U002AttentionNoBPModel, features: np.ndarray, targets: np.ndarray) -> np.ndarray:
    weights = model.output_weights.copy()
    for feature, target in zip(features, targets):
        logits = model.cfg.logit_scale * (weights @ feature) + model.output_bias
        target = int(target)
        weights[target] = phase.normalize_vector(weights[target] + model.cfg.output_lr * feature)
        adjusted = logits.astype(np.float32, copy=True)
        target_score = float(adjusted[target])
        adjusted[target] = -np.inf
        k = min(max(int(model.cfg.neg_k), 1), model.vocab_size - 1)
        wrongs = np.argpartition(adjusted, -k)[-k:]
        for wrong in wrongs:
            wrong = int(wrong)
            if float(adjusted[wrong]) + model.cfg.output_margin <= target_score:
                continue
            weights[wrong] = phase.normalize_vector(weights[wrong] - (model.cfg.output_lr / k) * feature)
    return weights


def run_center_diff_diag(
    model: U002AttentionNoBPModel,
    ids: np.ndarray,
    batch_size: int,
    eps: float,
) -> dict[str, float | int]:
    features, targets = collect_diag_batch(model, ids, batch_size)
    if features.shape[0] == 0:
        return {"diag_examples": 0}
    weights0 = model.output_weights.copy().astype(np.float64)
    bias = model.output_bias.copy()
    before = output_loss(weights0.astype(np.float32), bias, features, targets, model.cfg.logit_scale)
    ...
    def loss_fn(flat: np.ndarray) -> float:
        ...
        return output_loss(w, bias, features, targets, model.cfg.logit_scale)
    ...
    return {
        ...
        "loss_after_local": float(loss_fn((weights0 + local_delta).ravel())),
        "loss_after_center_diff_scaled": float(loss_fn((weights0 + cd_step).ravel())),
        "loss_after_random_scaled": float(loss_fn((weights0 + random_step).ravel())),
        "loss_change_local": float(loss_fn((weights0 + local_delta).ravel()) - before),
        "loss_change_center_diff_scaled": float(loss_fn((weights0 + cd_step).ravel()) - before),
        "loss_change_random_scaled": float(loss_fn((weights0 + random_step).ravel()) - before),
...
def local_block_step(
    ...
    for context, target in zip(contexts, targets):
        _, traces = model.forward(context, collect=True)
        trace = traces[block_idx]
        local_input = trace["input"]
        hidden = trace["hidden"]
        local_target = model.block_targets[block_idx][int(target)]
        delta = (local_target - hidden) * (1.0 - np.square(hidden))
        weights = weights + model.cfg.block_lr * np.outer(delta, local_input).astype(np.float32)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out-dir", type=Path, default=SCRIPT_DIR / "output" / "u002_attention_no_bp")
    ...
    parser.add_argument("--output-margin", type=float, default=0.0)
    ...
    parser.add_argument("--hidden-diag-max-params", type=int, default=16_384)
    parser.add_argument("--large-vocab-size", type=int, default=50_000)
    parser.add_argument("--large-d-model", type=int, default=1536)
    parser.add_argument("--large-blocks", type=int, default=8)
    parser.add_argument("--large-ff-mult", type=int, default=4)
...
def main() -> None:
    ...
    model = U002AttentionNoBPModel(vocab_size, cfg)
    large_estimate = estimate_transformer_like_params(
    ...
    diag = run_center_diff_diag(model, valid_ids, args.diag_batch, args.center_eps)
    hidden_diag_rows = run_block_center_diff_diag(
        model,
        valid_ids,
        args.hidden_diag_batch,
        args.center_eps,
        args.hidden_diag_max_params,
    )
    ...
    summary.update({f"diag_{key}": value for key, value in diag.items()})
    summary.update(hidden_diag_summary)
    write_csv(args.out_dir / "summary.csv", [summary])
    write_csv(args.out_dir / "center_diff_diagnostic.csv", [diag])
    ...
    print(
        ...
        f"diag_cos={diag.get('cosine_local_vs_center_diff', float('nan')):.3f} "
